...
public class MemberCreateRequest {
    ...
    @NotNull(message=ErrorMessage.PASSWORD_NOT_FOUND)
    @Size(min = 8, max = 20, message = ErrorMessage.LOGIN_PASSWORD_SIZE)
    private String password;

    @NotNull(message=ErrorMessage.PHONE_NUMBER_NOT_FOUND)
    @Pattern(regexp = "^010-\\d{4}-\\d{4}$", message = ErrorMessage.PHONE_NUMBER_PATTURN)
    private String phoneNumber;

    @NotNull(message=ErrorMessage.ADDRESS_NOT_FOUND)
